...
const SampdoriaQuiz = () => {
    // Usa il hook di Firebase per ottenere i dati
    ...
    // Mostra loading state mentre i dati vengono caricati
    if (loading) {
        return (
    ...
    // Mostra errori se ci sono problemi nel caricamento
    if (error) {
        return (
            <div className="min-h-screen flex items-center justify-center">
    ...
    const handleQuizComplete = (scores) => {
        console.log("Quiz completato, punteggi finali:", scores);
    ...
    // Il componente Setup rimane invariato
    ...
    const ModeSelection = ({ onSelectMode }) => {
        // Il componente ModeSelection rimane invariato
        return (
